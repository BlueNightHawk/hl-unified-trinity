...
====================
*/
void CParticleEngine::MsgCreateSystem(const char* pszName, BufferReader& reader) 
{
	Vector pos;
	pos = reader.ReadCoordVector();

	Vector ang;
	ang = reader.ReadCoordVector();

	int iType = reader.ReadByte();
	char* szPath = reader.ReadString();
	int iId = reader.ReadShort();
