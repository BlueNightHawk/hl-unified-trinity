...
====================
*/
int CBSPRenderer::FilterEntities(int type, struct cl_entity_t* pEntity, const char* modelname)
{
...
// draw stuff
	...
	if (c)
	{
		bool bIsLit = DynamicLighted(Vector(node->minmaxs[0], node->minmaxs[1], node->minmaxs[2]), Vector(node->minmaxs[3], node->minmaxs[4], node->minmaxs[5]));
...
void CBSPRenderer::RecursiveCreateDecal( mnode_t *node, decalgroupentry_t *texptr, customdecal_t *pDecal, Vector endpos, Vector pnormal )
{
	...
	if (CullDecalBBox(Vector(node->minmaxs[0], node->minmaxs[1], node->minmaxs[2]), Vector(node->minmaxs[3], node->minmaxs[4], node->minmaxs[5])))
		return;
...
====================
*/
void CBSPRenderer::MsgCustomDecal(const char* pszName, BufferReader& reader)
{
	Vector pos, normal;
	pos = reader.ReadCoordVector();
	normal = reader.ReadCoordVector();

	CreateDecal(pos, normal, reader.ReadString(), reader.ReadByte());
}
...
void CBSPRenderer::RecursiveWorldNodeLight( mnode_t *node )
{
	...
	if (!m_pCurrentDynLight->cone_size)
	{
		if (CullDynLightBBox(Vector(node->minmaxs[0], node->minmaxs[1], node->minmaxs[2]), Vector(node->minmaxs[3], node->minmaxs[4], node->minmaxs[5]))) // cull from point light bbox
			return;
	}
	else
	{
...
====================
*/
void CBSPRenderer::MsgSkyMarker_Sky(const char* pszName, BufferReader& reader)
{
	m_vSkyOrigin = reader.ReadCoordVector();

	gHUD.m_pSkyFogSettings.affectsky = true;
	gHUD.m_pSkyFogSettings.end = reader.ReadShort();
	gHUD.m_pSkyFogSettings.start = reader.ReadShort();
	gHUD.m_pSkyFogSettings.color.x = (float)reader.ReadByte() / 255;
	gHUD.m_pSkyFogSettings.color.y = (float)reader.ReadByte() / 255;
	gHUD.m_pSkyFogSettings.color.z = (float)reader.ReadByte() / 255;
	gHUD.m_pSkyFogSettings.affectsky = (reader.ReadShort() == 1) ? false : true;
...
====================
*/
void CBSPRenderer::MsgSkyMarker_World(const char* pszName, BufferReader& reader)
{
	m_vSkyWorldOrigin = reader.ReadCoordVector();
	m_fSkySpeed = reader.ReadCoord();
}
...
====================
*/
void CBSPRenderer::MsgDynLight(const char* pszName, BufferReader& reader)
{
	cl_dlight_t *dl = CL_AllocDLight(0);

	dl->origin = reader.ReadCoordVector();
	dl->radius = reader.ReadByte() * 10;
	dl->color.x = (float)reader.ReadByte() / 255;
	dl->color.y = (float)reader.ReadByte() / 255;
	dl->color.z = (float)reader.ReadByte() / 255;
	dl->die = reader.ReadByte() + gEngfuncs.GetClientTime();
	dl->decay = reader.ReadByte() * 10;
}
