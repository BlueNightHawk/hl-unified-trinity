...
public:
	...
	int FilterEntities(int type, struct cl_entity_t* pEntity, const char* modelname); //

	void DecayLights( void );
	bool HasDynLights( void );
	void GetAdditionalLights( void );
	cl_dlight_t *CL_AllocDLight( int key );
	void MsgDynLight(const char* pszName, BufferReader& reader);
	...
	void DrawSky( void );
	void RemoveSky ( void );
	void MsgSkyMarker_Sky(const char* pszName, BufferReader& reader);
	void MsgSkyMarker_World(const char* pszName, BufferReader& reader);
...
public:
	...
	void MsgCustomDecal(const char* pszName, BufferReader& reader);
