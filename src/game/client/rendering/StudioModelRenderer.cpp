...
mstudioanim_t* CStudioModelRenderer::StudioGetAnim(model_t* m_pSubModel, mstudioseqdesc_t* pseqdesc)
{
	...
	if (!IEngineStudio.Cache_Check((struct cache_user_t*)&(paSequences[pseqdesc->seqgroup])))
	{
		gEngfuncs.Con_DPrintf("loading %s\n", pseqgroup->name);
		IEngineStudio.LoadCacheFile(pseqgroup->name, (struct cache_user_t*)&paSequences[pseqdesc->seqgroup]);
	}
...
void CStudioModelRenderer::StudioSetupLighting(void)
{
	...
	if (!pInfo)
	{
		if (m_pCurrentEntity->index > 0 && m_pCurrentEntity != gEngfuncs.GetViewModel())
		{
			...
			m_pCurrentEntity->topnode = (mnode_t*)pInfo;
		}
	}
	else
	{
...
void CStudioModelRenderer::StudioSetChromeVectors(void)
{
	...
	for (int i = 0; i < m_pStudioHeader->numbones; i++)
	{
		...
		chromeupvec = CrossProduct(tmp, m_vRight);
		VectorNormalizeFast(chromeupvec);
		chromeupvec.z += flSin;
		chromerightvec = CrossProduct(tmp, chromeupvec);
...
void CStudioModelRenderer::StudioDrawExternalEntity(cl_entity_t* pEntity)
{
	...
	if (m_pCurrentEntity->curstate.renderfx == 70)
	{
		...
		{
			vTransOrigin = (m_pCurrentEntity->origin - gBSPRenderer.m_vSkyOrigin) + (Vector)m_vRenderOrigin;
...
studiodecal_t* CStudioModelRenderer::StudioAllocDecal(void)
{
	if (!m_pCurrentEntity->efrag)
	{
		...
		m_pCurrentEntity->efrag = (struct efrag_t*)pDecal;
		return pDecal;
	}
